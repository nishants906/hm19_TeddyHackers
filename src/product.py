...
class demo1:
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if ret:
                ...
                frame = self.process_frame(frame, er / 1.5, mr * 1.5)
                print('ear', er, "mr", mr)
    ...
    def process_frame(self, frame=None, eyethresh=0, mouththresh=0):
        ...
        cv2.putText(frame, "Geek Buzz", (20, 25),
        ...
        if len(faces_loc) > 0:
            ...
            did_yawn, _, _, _ = self.ddestimator.get_mouth_openess_over_time(mar_threshold=mouththresh)
            print("eyethresh", eyethresh, "mouththresh", mouththresh)
            #########################################
            ##########################################
            global alarm_frames_threshold, aft, last_alert, talking_frames_threshold, talking_theshold, right, left

            if mar >= mr + .05 and mar <= mouththresh + 0.5:
                talking_theshold += 2
            elif mr>mar:
                talking_theshold -=1
            else:
